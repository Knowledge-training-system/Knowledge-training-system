...
with open(questiontxtPath, 'r', encoding='UTF-8') as f:
    ...
    for line in f:
        apieceOfDumpData={} 
        apieceOfDumpData['model']='modelsApp.questionslist'
        apieceOfDumpData['pk']=len(piecesOfDumpData)+1
        if line == '\n':#为空行
            continue
        if line == '查看解析\n':
            continue
        ...
        if currentType==-1:
            ...
            if matchObj2 == None:
                ...
                currentQuestion['difficulty']=random.randint(1,10)
                isType2answer=0
            ...
            if matchObj3 == None:
                ...
                scores=[3,4]
                currentQuestion['score']=scores[currentType]
                #questionsList.append(currentQuestion)
                apieceOfDumpData['fields']=currentQuestion
                piecesOfDumpData.append(apieceOfDumpData)
                currentQuestion={}
                currentType=-1
                continue
            else:
                options.append(matchObj3.group(1).encode(encoding='UTF-8',errors='strict').decode('utf-8'))#
                if len(options)==4:
            ...
            if isType2answer==1:
                ...
                if len(type2Answers) == type2answerCount:
                    ...
                    currentQuestion['score']=type2answerCount*2
                    apieceOfDumpData['fields']=currentQuestion
                    piecesOfDumpData.append(apieceOfDumpData)
                    currentQuestion={}
                    currentType=-1
    ...
    resultPath = str(Path('./db.json'))
    with open(resultPath, 'w', encoding='UTF-8') as json_file:
        json_file.write(json.dumps(piecesOfDumpData, ensure_ascii=False, indent=1))
